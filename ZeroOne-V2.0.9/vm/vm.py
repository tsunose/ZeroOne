...
class ZeroOneVM:
    ...
    def run(self):
        ...
        while self.running and self.ip < len(self.code):
            ...
            try:
                ...
                # Never leak implementation-specific Python exceptions into
                # ZeroOne programs. Convert them to a VMError so TRY/CATCH
                # and the CLI see a stable language-level error.
                kind = type(exc).__name__.replace("Error", "").strip() or "Runtime"
                caught = VMError(f"{OpCode.name(opcode)}: {kind} error - {exc}")
    ...
    def execute(self, opcode, operand):
        ...
        if opcode == OpCode.RSHIFT:
            b = self.pop()
            a = self.pop()
            # Logical/unsigned right shift: 32-bit zero-fill semantics.
            self.push((int(a) & 0xFFFFFFFF) >> int(b))
            self.ip += 1
            return
        ...
        if opcode == OpCode.NEW:
            ...
            # Resolve init through the inheritance chain. A subclass inherits
            # its parent's constructor when it does not define its own.
            cursor = class_obj
            ctor = None
            seen = set()
            while isinstance(cursor, dict) and cursor.get("__name__") not in seen:
                seen.add(cursor.get("__name__"))
                methods = cursor.get("__methods__", {})
                if isinstance(methods, dict):
                    # `init` is the primary constructor name. `constructor`
                    # remains accepted for compatibility with the V2.0.9
                    # class syntax documentation.
                    if "init" in methods:
                        ctor = methods["init"]
                        break
                    if "constructor" in methods:
                        ctor = methods["constructor"]
                        break
                parent_name = cursor.get("__extends__")
                cursor = self.load_variable(str(parent_name)) if parent_name else None

            if ctor is not None:
                bound = dict(ctor)
                bound["__bound_this__"] = instance
                self._invoke_callable(bound, args)
            elif args:
                raise VMError(
                    f"Class '{class_obj.get('__name__')}' has no constructor but "
                    f"NEW received {len(args)} argument(s)."
                )
        ...
        if opcode == OpCode.GET_PROP:
            ...
            if value is None and isinstance(obj, dict) and obj.get("__kind__") == "instance":
                ...
                seen = set()
                while isinstance(cls, dict) and cls.get("__name__") not in seen:
                    seen.add(cls.get("__name__"))
                    methods = cls.get("__methods__")
                    if isinstance(methods, dict) and operand in methods:
                        value = methods[operand]
                        break
                    parent_name = cls.get("__extends__")
                    cls = self.load_variable(str(parent_name)) if parent_name else None
