...
Version 2.0.9 - Extended Parser (Fixed for v2.0.9 keywords)
"""
...
class Parser:
    ...
    def _expect_identifier_or_keyword(self):
        ...
        This allows v2.0.9 keywords like LENGTH, CHAR_AT, SIZE to be used as names.
        """
    ...
    def parse_set(self):
        ...
        # v2.0.9 fix: Allow KEYWORD tokens as variable names
        # This enables using v2.0.9 keywords (LENGTH, CHAR_AT, SIZE, etc.) as identifiers
    ...
    def parse_foreach(self):
        ...
        # v2.0.9 fix: Allow KEYWORD tokens as loop variable names
        ...
        while not self.is_end():
            ...
            if self.check_keyword("CATCH"):
                ...
                if self.check_symbol("("):
                    ...
                    if self.check_symbol(":"):
                        self.advance()
                        # v2.0.9 fix: Allow KEYWORD tokens for exception variable
                        variable = self._expect_identifier_or_keyword()
                    self.expect_symbol(")")
    ...
    def parse_function(self):
        ...
        # v2.0.9 fix: Allow KEYWORD tokens as function names
        name = self._expect_identifier_or_keyword()
        ...
        if self.check_symbol("("):
            self.advance()
            while not self.check_symbol(")"):
                # v2.0.9 fix: Allow KEYWORD tokens as parameter names
    ...
    def parse_class(self):
        ...
        # v2.0.9 fix: Allow KEYWORD tokens as class names
        name = self._expect_identifier_or_keyword()

        extends = None
        if self.check_keyword("EXTENDS"):
            self.advance()
            # v2.0.9 fix: Allow KEYWORD tokens for parent class names
    ...
    def parse_identifier_statement(self):
        ...
        # Excluded when the next token is `[`, `.`, or `=`: those mean
        # this identifier is being used as an ordinary variable (index/
        # property access, or a bare `name = value` reassignment without
        # SET), not as a call -- e.g. `size[0] = 1`, `size.x`, `size = 5`
        # for a variable happened to be named `size`. Without this check,
        # any of the ~250 KEYWORD_ALIASES words (env, size, length, sort,
        # first, last, max, min, push, pop, ...) used as a plain variable
        # name broke the moment it was indexed, accessed as a property,
        # or reassigned without SET -- the exact same class of reserved-
        # word collision problem 1 was meant to fix, resurfacing here via
        # a separate check that problem 1's lexer-level fix didn't cover.
        if (
            name.upper() in KEYWORD_ALIASES
            and not self.check(TokenType.NEWLINE)
            and not self.is_end()
            and not self.check_symbol("[")
            and not self.check_symbol(".")
            and not self.check_symbol("=")
        ):
            arguments = []
        ...
        # Array index / property access or assignment, chained to any depth
        # (e.g. `a.child.value = 5`, `a[0].b[1] = x`). Previously only a
        # single `.field` or `[index]` step was consumed before deciding
        # whether this was an assignment, so a second chained step (like
        # `.value` after `.child`) was left unconsumed and blew up the
        # parser as a stray top-level `.` token.
        while self.check_symbol("[") or self.check_symbol("."):

            if self.check_symbol("["):
                self.advance()
                index_expr = self.parse_expression()
                self.expect_symbol("]")
                target = IndexNode(target, index_expr)
                ...
                property_name = self.current.value
                self.advance()
                target = PropertyNode(target, property_name)

        if self.check_symbol("=") and isinstance(target, (PropertyNode, IndexNode)):
            self.advance()
            value = self.parse_expression()
            if isinstance(target, PropertyNode):
                return PropertySetNode(target.target, target.property_name, value)
            return IndexSetNode(target.target, target.index, value)
    ...
    def parse_postfix(self):
        ...
        while True:
            ...
            # Function/method call
            elif self.check_symbol("(") and isinstance(expr, (IdentifierNode, PropertyNode)):
                ...
                if isinstance(expr, IdentifierNode):
                    ...
                    expr = MethodCallNode(expr.target, expr.property_name, arguments)
    ...
    def parse_primary(self):
        ...
        # NEW expression: NEW ClassName(arg1, arg2)
        if self.check_keyword("NEW"):
            self.advance()
            if not (self.check(TokenType.IDENTIFIER) or self.check(TokenType.KEYWORD)):
                raise ParserError("Expected class name after NEW.")
            class_name = self.current.value
            self.advance()
            arguments = []
            if self.check_symbol("("):
                self.advance(); self.skip_newline()
                while not self.check_symbol(")"):
                    arguments.append(self.parse_expression()); self.skip_newline()
                    if not self.check_symbol(")"):
                        self.expect_symbol(","); self.skip_newline()
                self.expect_symbol(")")
    ...
    def parse_lambda(self):
        ...
        if self.check_symbol("("):
            ...
            while not self.check_symbol(")"):
                ...
                # v2.0.9 fix: Allow KEYWORD tokens as lambda parameter names
                param_name = self._expect_identifier_or_keyword()
