...
Version 2.0.4 - Extended Code Generator
"""
...
class Generator:
    ...
    def generate_expression(
        ...
        if isinstance(
            ...
            if str(node.name).upper() == "THIS":
                self.emit(OpCode.LOAD_LOCAL, "this")
            else:
        ...
        if isinstance(node, NewNode):
            if node.class_name not in self.class_table:
                raise GeneratorError(f"Unknown class '{node.class_name}'.")
            self.generate_expression(IdentifierNode(node.class_name))
        ...
        if isinstance(
            ...
            op_upper = str(node.operator).upper()

            # True short-circuit evaluation. The right-hand expression must
            # not execute when its result cannot affect the logical result.
            if op_upper in ("AND", "&&"):
                false_label = self.new_label()
                end_label = self.new_label()
                self.generate_expression(node.left)
                self.emit(OpCode.JMP_IF_FALSE, false_label)
                self.generate_expression(node.right)
                self.emit(OpCode.JMP, end_label)
                self.place_label(false_label)
                self.emit(OpCode.PUSH, False)
            ...
            if op_upper in ("OR", "||"):
                ...
                end_label = self.new_label()
                self.generate_expression(node.left)
                self.emit(OpCode.JMP_IF_TRUE, true_label)
                self.generate_expression(node.right)
                self.emit(OpCode.JMP, end_label)
            ...
            self.generate_expression(node.left)
            self.generate_expression(node.right)
            self.generate_binary(node.operator)
            return
        ...
        if isinstance(node, MethodCallNode):
            self.generate_expression(node.target)
            self.emit(OpCode.GET_PROP, node.name)
            for arg in node.arguments:
                self.generate_expression(arg)
            self.emit(OpCode.PUSH, len(node.arguments))
        ...
        if isinstance(
            ...
            name = node.name
            # STRUCT types are callable constructors.
            if name in self.struct_table:
                fields = self.struct_table[name]
                if len(node.arguments) > len(fields):
                    raise GeneratorError(
                        f"Struct '{name}' expects at most {len(fields)} argument(s), "
                        f"got {len(node.arguments)}."
                    )
                self.emit(OpCode.LOAD, name)
                for arg in node.arguments:
                    self.generate_expression(arg)
                self.emit(OpCode.PUSH, len(node.arguments) + 1)
                native_id = NATIVE_IDS["STRUCT_NEW"]
        ...
        if isinstance(node, BuiltinCallNode):
            ...
            if node.name.upper() in {"OUT","PRINT","SHOW","DISPLAY","ECHO"}:
                # These built-ins are void (their VM opcode consumes the
                # value to print and pushes nothing back). generate_statement
                # already knows this and skips the extra POP it otherwise
                # adds after a builtin call -- but here, as an *expression*
                # (e.g. the implicit return value of a single-expression
                # LAMBDA body, or any other place a value is expected), we
                # must supply a placeholder value ourselves. Without this,
                # a void LAMBDA body left nothing on the stack for RETURN to
                # return, which underflowed the stack of whatever called it
                # (a bare `f(1)` statement, or VISIT/MAP/FILTER/REDUCE's
    ...
    def generate_statement(
        ...
        if isinstance(node, EnumNode):
            self.enum_table[node.name] = node.members
            self.emit(OpCode.MAP_NEW)
            for member, value in node.members:
                self.emit(OpCode.PUSH, member)
                self.emit(OpCode.PUSH, value)
                self.emit(OpCode.MAP_SET)
            self.emit(OpCode.STORE_LOCAL if self.function_depth else OpCode.STORE, node.name)
            return

        if isinstance(node, StructNode):
            fields = [field[0] for field in node.fields]
            self.struct_table[node.name] = fields
            # Runtime descriptor: {"__kind__":"struct", "__name__":..., "fields":[...]}
            self.emit(OpCode.MAP_NEW)
            for key, value in [
                ("__kind__", "struct"),
                ("__name__", node.name),
            ]:
                self.emit(OpCode.PUSH, key)
                self.emit(OpCode.PUSH, value)
                self.emit(OpCode.MAP_SET)
            self.emit(OpCode.PUSH, "fields")
            self.emit(OpCode.ARRAY_NEW)
            for field in fields:
                self.emit(OpCode.PUSH, field)
                self.emit(OpCode.ARRAY_PUSH)
            self.emit(OpCode.MAP_SET)
            self.emit(OpCode.STORE_LOCAL if self.function_depth else OpCode.STORE, node.name)
            return
        ...
        if isinstance(node, ExportNode):
            # Exports are consumed by the compile-time module resolver.
            return
    ...
    def generate_try(self, node):
        ...
        # Normal completion closes only this TRY frame. This must not use
        # FINALLY itself, because nested TRY scopes may have an outer handler.
    ...
    def generate_class(self, node):
        # Runtime class descriptor with closures for methods.
        if node.name in self.class_table:
            raise GeneratorError(f"Class '{node.name}' already exists.")
        methods = {}
        for item in node.body:
            if isinstance(item, FunctionNode):
                if item.name in methods:
                    raise GeneratorError(f"Method '{item.name}' already exists in class '{node.name}'.")
                methods[item.name] = item
        self.class_table[node.name] = {"extends": node.extends, "methods": {k: True for k in methods}}
        skip_label = self.new_label()
        self.emit(OpCode.MAP_NEW)
        for key, value in [("__kind__", "class"), ("__name__", node.name), ("__extends__", node.extends)]:
            self.emit(OpCode.PUSH, key); self.emit(OpCode.PUSH, value); self.emit(OpCode.MAP_SET)
        self.emit(OpCode.PUSH, "__methods__"); self.emit(OpCode.MAP_NEW)
        for name, method in methods.items():
            self.emit(OpCode.PUSH, name)
            self.emit(OpCode.CLOSURE, {"target": f"CLASS_{node.name}_{name}", "params": [p.name for p in method.params]})
            self.emit(OpCode.MAP_SET)
        self.emit(OpCode.MAP_SET); self.emit(OpCode.STORE, node.name); self.emit(OpCode.JMP, skip_label)
        for name, method in methods.items():
            self.place_label(f"CLASS_{node.name}_{name}")
            self.function_depth += 1
            for param in reversed(method.params): self.emit(OpCode.STORE_LOCAL, param.name)
            for statement in method.body: self.generate_statement(statement)
            self.emit(OpCode.PUSH, None); self.emit(OpCode.RETURN)
            self.function_depth -= 1
        self.place_label(skip_label)
