...
Version 2.0.2 - Extended Code Generator
"""
...
class Generator:
    ...
    def generate_program(
        ...
        # Pre-register top-level function signatures so calls can be
        # validated even when the function definition appears later.
        for statement in node.statements:
            if isinstance(statement, FunctionNode):
                if statement.name in self.function_arity:
                    raise GeneratorError(
                        f"Function '{statement.name}' already exists."
                    )
                self.function_arity[statement.name] = len(statement.params)
