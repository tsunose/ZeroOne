...
class ZeroOneVM:
    ...
    def native_call(self, name, args):
        ...
        if n in {"TO_INT", "PARSE_INT"}:
            ...
            except ValueError as e: raise VMError(f"Invalid integer: {get(0)}") from e
        if n == "PARSE_FLOAT":
            try: return float(str(get(0, 0)).strip())
            except ValueError as e: raise VMError(f"Invalid float: {get(0)}") from e
        ...
        if n == "PACK":
            ...
            try:
                ...
                raise VMError(f"PACK error: {e}") from e
        if n == "UNPACK":
            import struct
            if len(a) < 2:
            ...
            try:
                ...
                raise VMError(f"UNPACK error: {e}") from e
