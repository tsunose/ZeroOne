...
def test_version():
    ...
    assert "ZeroOne 2.0.6" in p.stdoutt
...
EXIT
"""
    assert run_zo(src, "t_bits") == "8146-11244"
...
EXIT
"""
    assert run_zo(src, "t_bitops") == "832-2"
...
EXIT
"""
    assert run_zo(src, "t_try") == "caughtfinally"
...
EXIT
"""
    out = run_zo(src, "t_runtime")
    assert out
    assert "linux" in out.lower() or "windows" in out.lower() or "darwin" in out.lower()
