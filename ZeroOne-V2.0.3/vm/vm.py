...
Version 2.0.2 - Extended Virtual Machine
"""
...
class ZeroOneVM:
    ...
    def execute(self, opcode, operand):
        ...
        if opcode == OpCode.TRY:
            self.exception_handler = int(operand)
            self.ip += 1
            return
        ...
        if opcode == OpCode.FINALLY:
            # FINALLY itself must not discard a pending exception.  RETHROW,
            # emitted after the finally body, handles it explicitly.
            self.exception_handler = None
            self.ip += 1
            return
